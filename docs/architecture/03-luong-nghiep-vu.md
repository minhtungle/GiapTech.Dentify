...
## Cảnh báo trùng bệnh nhân khi thêm/sửa

```
1. Nhân viên nhập Họ tên (+ Số điện thoại nếu có) trong dialog thêm/sửa
   bệnh nhân, bấm "Lưu"
2. Frontend gọi GetDuplicatesAsync(fullName, phoneNumber, excludeId)
   TRƯỚC khi gọi create/update thật — excludeId = id bệnh nhân đang sửa
   (để không tự báo trùng với chính nó), không có khi tạo mới
3. Backend trả về tối đa 5 bệnh nhân có FullName trùng (không phân biệt
   hoa/thường) HOẶC PhoneNumber khớp chính xác — không throw exception,
   chỉ là dữ liệu tra cứu
4. Nếu danh sách rỗng → lưu ngay như bình thường, không thêm bước nào
5. Nếu có kết quả → dialog xác nhận hiện lên, liệt kê tên + SĐT các bệnh
   nhân trùng tìm thấy, hỏi "Bạn có chắc muốn tiếp tục lưu?"
6. Nhân viên bấm xác nhận → mới thật sự gọi create/update. Bấm huỷ →
   quay lại dialog form, không mất dữ liệu đã nhập (dialog form không tự
   đóng khi có cảnh báo)
```

Đây là cảnh báo hỗ trợ, không phải ràng buộc dữ liệu cứng — hệ thống vẫn cho phép tạo
nhiều bệnh nhân trùng tên/SĐT nếu nhân viên xác nhận cố ý (ví dụ 2 người thân dùng chung
số điện thoại). Nếu bản thân API `GetDuplicatesAsync` lỗi (mất kết nối, timeout), frontend
bỏ qua bước cảnh báo và lưu thẳng — không chặn nghiệp vụ chỉ vì tính năng hỗ trợ bị lỗi.
