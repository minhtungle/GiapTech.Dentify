...
| `CreateAsync` | validate `IdentityUserId` tồn tại (`EnsureIdentityUserExistsAsync`) và chưa gắn Doctor khác (`EnsureIdentityUserNotLinkedAsync` → throw `DoctorAlreadyLinkedToUser`) |
...
- **AppointmentsPage** (`/appointments`): Tabs "Bảng"/"Lịch". Bảng có bộ lọc đầy đủ (tên,
  ...
  `ChairId` bỏ qua (để trống) nếu không khớp thay vì chặn dòng. Không xuất
...
| `GetListAsync(GetTreatmentPlanListDto)` | phân trang, filter `PatientId`/`Status` |
...
| `GetMyBalanceAsync()` | tổng công nợ, tự tính lại trong scope Portal (không gọi chéo `PatientAppService`) |
